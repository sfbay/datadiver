...
interface DispatchHourlyPatternResult {
...
/**
 * Fetches hourly call pattern data for 911 dispatch (historical) dataset.
 * Uses received_datetime (not received_dttm like Fire/EMS).
 */
export function useDispatchHourlyPattern(
  dateRange: { start: string; end: string },
  extraWhereClause?: string
): DispatchHourlyPatternResult {
  const whereConditions: string[] = []
  whereConditions.push(`received_datetime >= '${dateRange.start}T00:00:00'`)
  whereConditions.push(`received_datetime <= '${dateRange.end}T23:59:59'`)
  ...
  const { data: rows, isLoading, error } = useDataset<HourlyAggRow>(
    ...
    {
      $select: 'date_extract_hh(received_datetime) as hour, date_extract_dow(received_datetime) as dow, count(*) as call_count',
      $group: 'hour, dow',
      $where: where,
