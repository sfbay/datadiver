...
describe('download URLs', () => {
  it('builds the CSV export from host + id + the same query string', () => {
    expect(csvUrl('data.sf.gov', 'wg3w-h783', '%24limit=5')).toBe('https://data.sf.gov/resource/wg3w-h783.csv?%24limit=5')
  })
  ...
  it('builds a resource geojson, never the dead geospatial export', () => {
    expect(geojsonUrl('data.sf.gov', 'j2bu-swwd', 100)).toBe('https://data.sf.gov/resource/j2bu-swwd.geojson?%24limit=100')
  })
  it('portal page uses the /d/ form', () => {
    expect(portalPageUrl('data.sf.gov', 'wg3w-h783')).toBe('https://data.sf.gov/d/wg3w-h783')
  })
})
