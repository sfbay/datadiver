...
export default function NetFormationChart({
  ...
  useEffect(() => {
    ...
    if (priorYear && priorYear.length > 0) {
      ...
      priorYear.forEach((d, i) => {
        ...
        g.append('rect')
          ...
          .attr('rx', 1.5).attr('fill', '#7a9954').attr('opacity', ghostOpacity)
        g.append('rect')
          .attr('x', xPos).attr('y', h / 2)
          .attr('width', bw).attr('height', yDown(d.closures) - h / 2)
          .attr('rx', 1.5).attr('fill', '#b85545').attr('opacity', ghostOpacity)
      })
    }
    ...
    g.append('text')
      ...
      .attr('fill', '#7a9954').style('font-size', '0.4375rem')
      .attr('font-family', '"JetBrains Mono", monospace')
      .text(`+${maxVal}`)
    g.append('text')
      .attr('x', -4).attr('y', h)
      .attr('text-anchor', 'end')
      .attr('fill', '#b85545').style('font-size', '0.4375rem')
      .attr('font-family', '"JetBrains Mono", monospace')
      .text(`-${maxVal}`)
