...
export const detectSignificantEvents: Detector = (ctx: DetectorContext) => {
  ...
  for (const e of ctx.events) {
    ...
    out.push({
      ...
      source: { view: '/live', label: `${what} · ${where}` },
  ...
  return ctx.anomalies
    ...
    .map((a) => {
      ...
      return {
        ...
        source: { view: '/live', label: `${a.neighborhood} · ${stream}` },
        priority: Math.round(score),
        score,
...
export const detectStreamRateSpike: Detector = (ctx) => {
  ...
  for (const [datasetId, evs] of byDataset) {
    ...
    out.push({
      ...
      category: 'trend',
      severity: 'negative',
      source: { view: '/live', label: humanizeStreamName(datasetId) },
      priority: Math.round(score),
      score,
      intent: { type: 'none' },
      ...base(ctx.now),
...
export const detectRepeatedType: Detector = (ctx) => {
  ...
  for (const { plural, n } of counts.values()) {
    ...
    out.push({
      ...
      category: 'anomaly',
      severity: 'alert',
      source: { view: '/live', label: `${n} ${plural}` },
      priority: Math.round(score),
      score,
      intent: { type: 'none' },
      ...base(ctx.now),
