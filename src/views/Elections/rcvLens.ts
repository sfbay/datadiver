...
// (transfer-flow analysis), WHAT-IF (strike-a-candidate re-tabulation). All
// three are shipped as of 2026-07-21.

export type RcvLens = 'replay' | 'coalition' | 'whatif'

export const ALL_LENSES: readonly RcvLens[] = ['replay', 'coalition', 'whatif']

/** Lenses with a shipped UI. Keep in sync as future lenses land. */
 ...
 *   - a KNOWN-but-not-yet-shipped lens — also null, so a deep link to an
 *     unbuilt lens degrades gracefully to the default view instead of
 *     rendering nothing or throwing. This is why the check consults
 *     SHIPPED_LENSES, not ALL_LENSES — the mechanism stays live for
 *     whatever lens comes next. */
