...
```ts
import { describe, it, expect } from 'vitest'
import { ERA_SOURCES, eraSourceForPath, buildEraQuery, eraDomain } from './eraSources'
...
describe('buildEraQuery', () => {
  it('groups by year with an open upper bound when unclamped', () => {
    const q = buildEraQuery(ERA_SOURCES['crime-incidents'])
    expect(q.$select).toBe('date_extract_y(incident_datetime) as yr, count(*) as n')
    expect(q.$group).toBe('yr')
    expect(q.$where).toBe("incident_datetime >= '2003-01-01'")
  })
...
/** The annual GROUP BY. ~24 rows; the caller caches it hard. */
export function buildEraQuery(src: EraSource): {
  $select: string; $group: string; $where: string; $limit: number
} {
  const [lo, hi] = src.clamp
  const where = [`${src.dateField} >= '${lo}-01-01'`]
...
export function useEraSeries(pathname: string): UseEraSeriesResult {
  ...
  const years = useMemo(() => (source ? parseYearCounts(data) : []), [data, source])
  ...
  return {
    ...
    isLoading: source != null && isLoading,
