...
### 8.4 One motion, the orbit as the clock, and the navigator (2026-09-23, PRs #180 + #181)

Jesse's walks of Sept. 22–23. Every cause below was MEASURED in Chrome with a Beacon-position recorder (the stop's screen y every 250 ms) and a leg trace, after a first fix built from code reading did not hold.

- **"Double dip" on arrival** had two causes. (1) The take-off aim read `tileset.getHeight` at a place whose tiles were not resident (92 m off on Nob Hill), so a re-aim glide followed the landing — each leg now samples the TRUE ground first (`sampleHeightMostDetailed`, entities excluded, ≤ 2.5 s, overlapping the 3 s turn). (2) Cesium lerps heading/pitch apart from its arc, so the stop slid past the bottom edge mid-flight and the camera tipped back onto it — `holdAim` (scene.preUpdate) now keeps the stop's GROUND point on its final screen spot through the approach. Measured after: the pin eases into place in ~4 s, then holds within 4 px.
- **Arrivals "tilting down and dropping all at the end"**: Cesium's parabola fell steepest at the very end. The flight's height is now `glideHeight` — climb 20°, cruise ≤ 700 m, then glide in down the arrival's own line of sight (closer without tilting).
- **The orbit times the stop** (Jesse: "a full rotation to time each stop" / "the rotation in the direction of our takeoff"): each dwell sweeps between a half and a full circle and ends FACING THE NEXT STOP (`orbitSweepDeg`) — the camera then sits on the line from the next stop through this one, so take-off needs no turn (measured: heading unchanged into the next leg). Constant speed, so the sweep is the dwell's length. 3.6°/s exceeded Jesse's "dreamstate"; he settled 1.3°/s with the `?orbit=` knob. One flyTo across a wide sweep flew the CHORD (the pin wandered ~34 px), so the orbit is a chain of 2.5 s flights (`sliceEase` keeps one speed profile). The orbit starts only once the tiles have loaded ("I thought we don't start hero until tiles settle?").
- **Navigator rail** ("right bar becomes a geographic navigator … needs a search box"): RETURN at the top; Play/Hold/Hide one row, key hints under the names; search · Places · all 41 Neighborhoods (the flat map's own `NEIGHBORHOOD_VIEWS` cameras, `?nbhd=`, counts "in last 48" in each stream's own unit) · Hotspots; Mapbox address search under Product Terms §2.7 (temporary geocodes, never stored → address detours are page state; no POI; no lat/long shown). The stop ledger moved to the band header; a detour reads "At … · Reset view", and Escape resets before it leaves.
- **Tile heights lie on unloaded tiles** (−25,289 m measured): every reader goes through `groundHeight.ts`.
